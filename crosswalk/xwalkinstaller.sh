...
[ $MINIFYSWITCH -eq 1 ] && mvn install:install-file -DgroupId=org.xwalk -DartifactId=xwalk_core_library_canary -Dversion=$xwalkver -Dpackaging=aar -Dfile=crosswalk-$xwalkver-arm.aar -DgeneratePom=true || \
  mvn install:install-file -DgroupId=org.xwalk -DartifactId=xwalk_core_library_canary -Dversion=$xwalkver -Dpackaging=aar -Dfile=crosswalk-$xwalkver.aar -DgeneratePom=true
